...
#define ACE_AES_SW_AES_BLOCK_SIZE	16

#define CONFIG_ACE_SWAES_FOR_SMALLBLOCK
#define CONFIG_ACE_BC_ASYNC
#define CONFIG_ACE_BC_IRQMODE
...
#define CONFIG_ACE_WATCHDOG_MS		100
#define s5p_ace_print_time(_msg_, _ts_)	\
	printk(KERN_INFO "%s%5lu.%06lu\n",	\
		(_msg_), (_ts_).tv_sec, (_ts_).tv_usec)
#define s5p_ace_dump(expr)	\
	do {	\
...
static void s5p_ace_deferred_clock_disable(struct work_struct *work)
{
	...
	if (tmp == 0) {
		...
		printk(KERN_NOTICE "ACE clock OFF\n");
#endif
	}
}
...
#if defined(CONFIG_ACE_DEBUG)
			printk(KERN_NOTICE "ACE clock ON\n");
#endif
		}
				...
				u8 *out, const u8 *in, u32 len, int irqen)
{
...
#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "AES: %s, in: 0x%08X, out: 0x%08X, len: 0x%08X\n",
			__func__, (u32)in, (u32)out, len);
	printk(KERN_NOTICE "AES: %s, AES_control : 0x%08X\n",
	...
	reg |= 0x3 << ACE_FC_BRDMACARCACHE_OFS; /* ARCACHE value */
#endif
...
#endif
#if defined(CONFIG_ACE_USE_ACP)
	reg |= 0x3 << ACE_FC_BTDMACAWCACHE_OFS; /* AWCACHE value */
#endif
	s5p_ace_write_sfr(ACE_FC_BTDMAC, reg);

	/* Set DMA */
	s5p_ace_write_sfr(ACE_FC_BRDMAS, in);
	s5p_ace_write_sfr(ACE_FC_BTDMAS, out);
				...
				u8 *out, const u8 *in, u32 len)
{
	...
	dma_map_single(NULL, (void *)in, len, DMA_TO_DEVICE);
	dma_map_single(NULL, (void *)out, len, DMA_FROM_DEVICE);
#else
	dmac_clean_range((void *)in, (void *)in + len);
	dmac_flush_range((void *)out, (void *)out + len);
#endif
#endif
...
#if !defined(CONFIG_ACE_USE_ACP)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)
	dma_unmap_single(NULL, (dma_addr_t)in, len, DMA_TO_DEVICE);
	dma_unmap_single(NULL, (dma_addr_t)out, len, DMA_FROM_DEVICE);
#endif
#endif
 ...
 * - size should be a multiple of ACE_AES_SW_AES_BLOCK_SIZE.
 */
...
#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "%s - %s (size: %d / %d)\n", __func__,
...
#else
	u8 *src, *dst;
	u8 tmpin[ACE_AES_SW_AES_BLOCK_SIZE];
	u8 tmpout[ACE_AES_SW_AES_BLOCK_SIZE];
	size_t count_in, count_out;
	int ret;

#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "%s - (size: %d / %d)\n", __func__,
		size, sctx->total);
#endif

	if (size > ACE_AES_SW_AES_BLOCK_SIZE)
...
#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "size: 0x%x, cin: 0x%x, cout 0x%x\n",
...
#if !defined(CONFIG_ACE_USE_ACP)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)
	dma_map_single(NULL, (void *)tmpin, size, DMA_TO_DEVICE);
	dma_map_single(NULL, (void *)tmpout, size, DMA_FORM_DEVICE);
#else
	dmac_clean_range((void *)tmpin, (void *)tmpin + size);
	dmac_clean_range((void *)tmpout, (void *)tmpout + size);
#endif
#endif
...
#if !defined(CONFIG_ACE_USE_ACP)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)
	dma_unmap_single(NULL, (dma_addr_t)src, size, DMA_TO_DEVICE);
	dma_unmap_single(NULL, (dma_addr_t)dst, size, DMA_FROM_DEVICE);
#else
	dmac_inv_range((void *)tmpout, (void *)tmpout + size);
#endif
#endif
...
#if defined(CONFIG_ACE_DEBUG)
		printk(KERN_NOTICE "total_start: %d (%d)\n",
				sctx->total, count);
		printk(KERN_NOTICE " in(ofs: %x, len: %x), %x\n",
				sctx->in_sg->offset, sg_dma_len(sctx->in_sg),
				sctx->in_ofs);
		printk(KERN_NOTICE " out(ofs: %x, len: %x), %x\n",
				sctx->out_sg->offset, sg_dma_len(sctx->out_sg),
				sctx->out_ofs);
#endif

		if (count > ACE_AES_SW_AES_BLOCK_SIZE)
			break;

		count = min(sctx->total, (size_t)ACE_AES_SW_AES_BLOCK_SIZE);
...
#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "  phys(src: %x, dst: %x)\n",
			(u32)src, (u32)dst);
#endif
...
#if !defined(CONFIG_ACE_USE_ACP)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)
	dma_map_single(NULL, (void *)sctx->src_addr, count, DMA_TO_DEVICE);
	dma_map_single(NULL, (void *)sctx->dst_addr, count, DMA_FROM_DEVICE);
#else
...
static int s5p_ace_aes_crypt_dma_wait(struct s5p_ace_device *dev)
{
	...
	src = (u8 *)virt_to_phys(sctx->src_addr);
	dst = (u8 *)virt_to_phys(sctx->dst_addr);
...
#if !defined(CONFIG_ACE_USE_ACP)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)
	dma_unmap_single(NULL, (dma_addr_t)src, sctx->dma_size,
		DMA_TO_DEVICE);
	dma_unmap_single(NULL, (dma_addr_t)dst, sctx->dma_size,
		DMA_FROM_DEVICE);
#else
	dmac_inv_range((void *)sctx->dst_addr,
...
#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "total_end: %d\n", sctx->total);
#endif
...
#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "get new req\n");
#endif
...
#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "%s (total: %d, dma_size: %d)\n", __func__,
				sctx->total, sctx->dma_size);
#endif
...
#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "%s (nbytes: 0x%x, mode: 0x%x)\n",
			   ...
			   unsigned int nbytes, int encmode)
{
...
#if defined(CONFIG_ACE_SWAES_FOR_SMALLBLOCK)
	if (nbytes <= ACE_AES_SW_AES_BLOCK_SIZE) {
	...
	if ((sctx->sfr_ctrl & ACE_AES_OPERMODE_MASK) != ACE_AES_OPERMODE_ECB)
		memcpy(sctx->sfr_semikey, walk.iv, AES_BLOCK_SIZE);
	...
	while ((nbytes = walk.nbytes)) {
		...
		u32 len = nbytes - (nbytes % AES_BLOCK_SIZE);
		u32 lastblock = len - AES_BLOCK_SIZE;
		u8 tmp[AES_BLOCK_SIZE];

		if (src == dst)
			memcpy(tmp, src + lastblock, AES_BLOCK_SIZE);

		s5p_ace_resume_device(&s5p_ace_dev);
		while (test_and_set_bit(FLAGS_BC_BUSY, &s5p_ace_dev.flags))
			udelay(1);
		s5p_ace_aes_run_engine(sctx, dst, src, len);
		clear_bit(FLAGS_BC_BUSY, &s5p_ace_dev.flags);

		if (src == dst)
			s5p_ace_aes_update_semikey(sctx,
					tmp, dst + lastblock, len);
		else
			s5p_ace_aes_update_semikey(sctx,
					src + lastblock, dst + lastblock, len);
...
const unsigned char sha1_digest_emptymsg[SHA1_DIGEST_SIZE] = {
				...
				u8 *out, const u8* in, u32 len)
{
...
#if defined(CONFIG_ACE_DEBUG)
	printk(KERN_NOTICE "Out: 0x%08X, In: 0x%08X, Len: %d\n",
				(u32)out, (u32)in, len);
	printk(KERN_NOTICE "PreLen_Hi: %u, PreLen_Lo: %u\n",
...
#endif
#if defined(CONFIG_ACE_USE_ACP)
	reg |= 0x3 << ACE_FC_HRDMACARCACHE_OFS; /* ARCACHE value */
#endif
	...
	if (sctx->prelen_low != 0 || sctx->prelen_high != 0) {
		...
		/* First block */
		if ((out != NULL) && (len == 0)) {
			/* ACE H/W does not compute hash for empty string */
			memcpy(out, sha1_digest_emptymsg, SHA1_DIGEST_SIZE);
			return 0;
		}
	}
	...
	/* Set HRDMA */
	s5p_ace_write_sfr(ACE_FC_HRDMAS, virt_to_phys((void *)in));
...
#if !defined(CONFIG_ACE_USE_ACP)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)
	dma_unmap_single(NULL, virt_to_phys((void *)in), len, DMA_TO_DEVICE);
#endif
#endif
			      ...
			      const u8 *data, unsigned int len)
{
	...
	/* Quick path */
	if ((partlen == 0) && ((len & (SHA1_BLOCK_SIZE - 1)) == 0)) {
		if (len > SHA1_BLOCK_SIZE)
			ret = s5p_ace_sha1_engine(sctx, NULL, src,
						len - SHA1_BLOCK_SIZE);
		if (len > 0) {
			memcpy(sctx->buffer, src, SHA1_BLOCK_SIZE);
			sctx->buflen = SHA1_BLOCK_SIZE;
		}
		goto out;
	}
	...
	if (partlen != 0) {
		if (partlen + len <= SHA1_BLOCK_SIZE) {
			...
			partlen = SHA1_BLOCK_SIZE - partlen;
			memcpy(sctx->buffer + partlen, src, partlen);
	...
	partlen = (partlen == 0 ? SHA1_BLOCK_SIZE : partlen);
	len -= partlen;
	if (len > 0)
...
{
	s5p_ace_sha1_init(desc);
	s5p_ace_sha1_update(desc, data, len);
	s5p_ace_sha1_final(desc, out);
...
static int __init s5p_ace_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_ACE_USE_ACP)
	ace_sss_user_con = ioremap(PA_SSS_USER_CON & PAGE_MASK, SZ_4K);
	if (ace_sss_user_con == NULL) {
	...
	writel(0x101, ace_sss_user_con + (PA_SSS_USER_CON & (PAGE_SIZE - 1)));

	iounmap(ace_sss_user_con);	/* This addr is not used any more. */
#endif
